                        ...
                        <table class="table-auto w-full text-left whitespace-no-wrap">
                            ...
                            @foreach($medias as $media)
                                <tr>
                                    ...
                                    <td class="px-4 py-3">{{ $media->title }}</td>
                                    <td class="px-4 py-3">
                                        <x-ahref :href="$media->url">{{ $media->url }}</x-ahref>
                                    </td>
                                    <td class="px-4 py-3">
                                        {{ $media->is_active }}
                                    </td>
                                    <td class="flex space-x-10">
                                        <x-ahref :href="route('admin.medias.edit', $media->id)">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </x-ahref>
                                        <x-ahref class="bg-red-500" :href="route('admin.medias.destroy', $media->id)">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
