            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="lg:max-w-lg max-w-md overflow-auto">
                        ...
                        <form method="POST" action="{{ route('admin.medias.update', $media->id) }}" class="space-y-6">
                            @csrf
                            @method('PUT')
                            <div>
                                ...
                                <select name="mood_id" id="mood_id" class="form-select block w-full mt-1 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded" required>
                                    ...
                                    @foreach($moods as $mood)
                                        <option value="{{ $mood->id }}" {{ $mood->id == $media->mood_id ? 'selected' : null  }}> {{ $mood->mood }} </option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <x-input id="url" class="block mt-1 w-full" type="text" name="url" :value="$media->url" required/>
                            </div>
                            <div>
                                <x-label for="title" :value="__('Title')" />

                                <x-input id="title" class="block mt-1 w-full" type="text" name="title" :value="$media->title" required/>
                            </div>
